...
#ifdef IN_FAASM
inline void pinnearmap_phase(char* s){}
#include <ndpapi.h>
#else
#include "stublib.h"
#endif
...
int ndp_reads() {
  pinnearmap_phase("read-input");
  m = getpdb( filename, NULL );
  fprintf(stderr, "getpdb done\n");

  /* Get the molecular force field from the .prm file. */
  readparm( m, filename2 );
  fprintf(stderr, "readparm done\n");
  pinnearmap_phase("read-input-done");
  return 0;
}
...
int main( int argc, char *argv[] )
{
  ...
  strcat(filename2, "/");
  strcat(filename2, argv[1]);
  strcat(filename2, ".prm");
  if( mytaskid == 0) printf( "Reading .pdb file (%s)\n", filename);

  #ifdef IN_FAASM
  __faasmndp_storageCallAndAwait(ndp_reads);
  #else
  ndp_reads();
  #endif
